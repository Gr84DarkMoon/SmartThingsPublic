 ...
 *
 *  Based on FortrezZ MimoLite Divice Handler
 */
metadata {
	// Automatically generated. Make future change here.
	definition (name: "MIMOlite Smoke Detector", namespace: "Gr84DarkMoon", author: "Gr84DarkMoon") {
	...
	tiles (scale: 2) {
        standardTile("switch", "device.switch", width: 2, height: 2, canChangeIcon: false, decoration: "flat") {
         ...
         standardTile("contact", "device.contact", width: 4, height: 4, inactiveLabel: false) {
			state "open", label: "clear", icon: "st.alarm.smoke.smoke", backgroundColor: "#33cc33"
			state "closed", label: "smoke", icon: "st.alarm.smoke.smoke", backgroundColor: "#FF0000"
		}
        standardTile("refresh", "device.switch", width: 2, height: 2, inactiveLabel: false, decoration: "flat") {
			state "default", label:'', action:"refresh.refresh", icon:"st.secondary.refresh"
		}
        standardTile("powered", "device.powered", width: 2, height: 2, inactiveLabel: false) {
			state "powerOn", label: "Power On", icon: "st.switches.switch.on", backgroundColor: "#ccff33"
			state "powerOff", label: "Power Off", icon: "st.switches.switch.off", backgroundColor: "#ff6600"
		}
		...
		main (["contact", "switch"])
		details(["contact", "switch", "alarm", "voltage", "powered", "refresh","configure"])
	}
}
...
log.debug "switchBinaryReport ${cmd}"
    ...
    {
		return createEvent(name: "switch", value: "on") // change switch value to on
    }
    else // if the switch sensor report says its off then do...
    {
		return createEvent(name: "switch", value: "off") // change switch value to off
    }
...
{
	log.debug "sent a AlarmReport command"
...
def sensorValueEvent(Short value) {
    ...
    def evt2 = null
	if (value) {
		evt1 = createEvent(name: "contact", value: "open", descriptionText: "$device.displayName is clear")
        evt2 = createEvent(name: "smoke",   value: "clear", descriptionText: "$device.displayName smoke is clear")
	} else {
		evt1 =  createEvent(name: "contact", value: "closed", descriptionText: "$device.displayName detects smoke")
        evt2 = createEvent(name: "smoke",   value: "detected", descriptionText: "$device.displayName smoke was detected!")
	}
    return [evt1, evt2]
}